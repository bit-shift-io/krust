...
async fn get_or_create_session(state: &AppState, session_id: &str) -> Arc<Session> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        while let Ok(n) = reader.read(&mut buffer) {
            ...
            // Use "fire-and-forget" send - if receivers lag, we drop the frame
            // to prevent unbounded growth. The broadcast channel capacity of 512
            // limits how many frames can be pending, and we don't retry on lag.
            let _ = tx_clone.send(data);

            // Track lag for potential metrics/backpressure
            lag_count = 0; // reset on successful send
        }
    });
